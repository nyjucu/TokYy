...
class ResCBAMBlock( nn.Module ):
    def __init__( self, in_channels, out_channels, dilation = 1 ):
        ...
        self.conv = nn.Sequential(
            ...
            nn.BatchNorm2d( out_channels ),
            nn.ReLU( inplace = True ),
            nn.Conv2d( out_channels, out_channels, 3, padding = dilation, stride = 1, bias = False, dilation = dilation ),
            nn.BatchNorm2d( out_channels ),
        )
...
class FeaturePyramidNetwork( nn.Module ):
    ...
    def forward( self, c, p = None ):
        c_conv = self.lateral_conv( c )

        if p is None:
            p_upsampled = torch.zeros_like( c_conv )
        else:
            p_upsampled = F.interpolate( p, scale_factor = 2, mode = 'nearest' )
...
class ResCBAMEncoder( nn.Module ):
    def __init__( self ):
        super().__init__()
        ...
        self.do1 = nn.Dropout2d( p = 0.20 )
        self.do2 = nn.Dropout2d( p = 0.22 )
        self.do3 = nn.Dropout2d( p = 0.25 )
        self.do4 = nn.Dropout2d( p = 0.28 )
        self.do5 = nn.Dropout2d( p = 0.30 )
...
class ResCBAMDecoderBlock( nn.Module ):
    def __init__( self, in_channels, skip_channels, out_channels, upsample = True ):
        ...
        self.resconv = ResCBAMBlock( out_channels + skip_channels, out_channels )

    def forward( self, x, skip ):
        x_up = self.upsamp( x ) if self.upsample else self.conv( x )
        x_cat = torch.cat( [ skip, x_up ], dim = 1 ) 
        return self.resconv( x_cat )


class ResCBAMUNet( nn.Module ):
    def __init__( self ):
        super().__init__()

        self.encoder = ResCBAMEncoder()
        
        self.decoder1 = ResCBAMDecoderBlock( 1024, 512, 512 )
        self.decoder2 = ResCBAMDecoderBlock( 512, 256, 256 )
        self.decoder3 = ResCBAMDecoderBlock( 256, 128, 128 )
        self.decoder4 = ResCBAMDecoderBlock( 128, 64, 64 )

        self.output = nn.Conv2d( 64, 1, kernel_size = 1 )

        self.do1 = nn.Dropout2d( p = 0.28 )
        self.do2 = nn.Dropout2d( p = 0.25 )
        self.do3 = nn.Dropout2d( p = 0.22 )
        self.do4 = nn.Dropout2d( p = 0.20 )
...
class AtrousCBAMEncoder( nn.Module ):
    def __init__( self ):
        super().__init__()
        ...
        self.l2 = ResCBAMBlock( 64, 128 )
        self.l3 = ResCBAMBlock( 128, 256 )
        self.l4 = ResCBAMBlock( 256, 512, dilation = 2 )
        self.l5 = ResCBAMBlock( 512, 1024, dilation = 4 )
        self.pool = nn.MaxPool2d( 2, 2 )
...
class AtrousResCBAMUNet( nn.Module ):
    def __init__( self ):
        super().__init__()

        self.atrous_encoder = AtrousCBAMEncoder()

        self.decoder1 = ResCBAMDecoderBlock( 1024, 512, 512, upsample = False )
        self.decoder2 = ResCBAMDecoderBlock( 512, 256, 256, upsample = False )
        self.decoder3 = ResCBAMDecoderBlock( 256, 128, 128 )
        self.decoder4 = ResCBAMDecoderBlock( 128, 64, 64 )
        self.output = nn.Conv2d( 64, 1, kernel_size = 1 )

    def forward( self, x ):
        x1, x2, x3, x4, x5 = self.atrous_encoder( x )
...
class Encoder( nn.Module ):
    def __init__( self ):
        ...
        self.l1 = ResBlock( 3, 64 )
        self.l2 = ResBlock( 64, 128 )
        self.l3 = ResBlock( 128, 256 )
        self.l4 = ResBlock( 256, 512 )
        self.l5 = ResBlock( 512, 1024 )
        self.pool = nn.MaxPool2d( 2, 2 )
